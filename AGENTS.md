...
- **Theme reality:** warm dawn (light) / dusk (dark) themes, fully implemented via `next-themes` + CSS tokens in `globals.css` (`:root` / `.dark`); toggle in the navbar. Design and audit against BOTH themes; style with `var(--*)` tokens, never hardcoded hexes. Fraunces serif (`font-display`) for headings; numbered `SectionHeading` component for section headers.
