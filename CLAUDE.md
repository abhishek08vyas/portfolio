...
- **Theme system (navy + amber, light + dark)**: design tokens are CSS custom properties in `src/app/globals.css` on `:root` (light: warm ivory `#faf7f1`, brand-navy text `#142240`, amber accent `#d97706`) and `.dark` (deep navy `#0d1428`, amber accent `#fbbf24`) — `--surface-page/card/raised`, `--edge`, `--text-strong/body/dim`, `--accent(-strong/-soft)`, `--glow-a/b`, `--shadow-soft/hover`, `--focus-ring`. Style components with these vars (`text-[var(--text-body)]` etc.), NOT hardcoded hexes. Amber CTAs/badges use navy text (`text-[#142240]` on `from-[#fbbf24] to-[#d97706]`) for contrast.
...
- Brand navy `#142240`/`#3D5176` is now first-class again: page text (light), gradient panels (contact band, experience card headers, footer), and text on amber CTAs. `tailwind.config.ts`/`src/lib/theme.ts`/`getTechColor()` still carry the legacy palette for chips/wordmark.
